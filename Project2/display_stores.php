...
$place = new stdClass; //SETS OBJECT
$places = array(); //THIS WILL BE THE ARRAY OF ALL OBJECTS
...
//OBJdata is where all objects that get passed over from php side
var arrayLen = OBJdata.length;  //gets the length to use as array length

console.log(OBJdata); //checker to make sure its passed correctly
...
while ( x < arrayLen ){  //DYNAMICLY ADD TO MAP, THIS WILL ALSO CHANGE DEPENDING ON HOW MANY ENTRIES THERE ARE
MarkerLocations.push(  [OBJdata[x].Id , OBJdata[x].Name , OBJdata[x].Latitude ,
                        OBJdata[x].Longitude , OBJdata[x].Address , OBJdata[x].City
                        ,OBJdata[x].State , OBJdata[x].Zipcode]  );
...
for (m = 0; m < MarkerLocations.length; m++) {    //INSTANCIATES THE POINT ON MAP
  console.log("Length of markerlocation " + MarkerLocations.length);  //THE LENGTH IS DYNAMIC AND ADJUSTS DEPENDING ON HOW MANY ENTRIES THERE ARE
